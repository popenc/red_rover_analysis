...
def main():
    ...
    cy = np.arange(0, 10, 0.5)
    cx = [0.1*(np.random.random() - 0.5) for i in cy]

    # points_data = open('points_of_interest/turn_tests_straight_line.csv', 'r', encoding='utf-16')
    # points_data = points_data.read().split('\n')
    # xarr, yarr = [], []
    # for i in range(0, len(points_data) - 1):
    #     xarr.append(float(points_data[i].split(',')[0]) / 10**6)
    #     yarr.append(float(points_data[i].split(',')[1]) / 10**6)
    # cx = xarr
    # cy = yarr
    ...
    state = unicycle_model.State(x=-1.0, y=-3.0, yaw=0.0, v=0.0)
    # state = unicycle_model.State(x=x0 - 2.0*x0, y=y0 - 2.0*y0, yaw=0.0, v=0.0)
    # state = unicycle_model.State(x=0, y=0, yaw=0.0, v=0.0)
